...
export default function ChatList({
  ...
  const [isSearchChatEmpty, setIsSearchChatEmpty] = useState(false);
  ...
  const handleChatSearch = async (searchVal) => {
    if (!searchVal.trim()) {
      setIsSearchChatEmpty(false);
      setIsChatSearched(false);
      return;
    }
    const searchedChatRef = ref(database, `chats/${userId}`);
    try {
      const snapshot = await get(searchedChatRef);
      if (snapshot.exists()) {
        const searchChatData = Object.entries(snapshot.val())
          .map(([id, chat]) => ({
            id,
            ...chat,
          }))
          .filter((chat) =>
            chat.name.toLowerCase().includes(searchVal.toLowerCase())
          );

        if (searchChatData.length === 0) {
          setIsSearchChatEmpty(true);
        } else {
          setIsSearchChatEmpty(false);
          setSearchedChat(searchChatData);
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        <div className="">
          {!isSearchChatEmpty &&
            (IsChatSearched ? sortchats(searchedChat) : sortchats(chats)).map(
              (chat) => (
                <div
                  key={chat.id}
          ...
          {isSearchChatEmpty && (
            <>
              <p className="text-center text-textLight">No chats found</p>
            </>
          )}
        </div>
      </div>
